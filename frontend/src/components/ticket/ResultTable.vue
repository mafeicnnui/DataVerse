<template>
  <div class="tq-table-fixed" v-if="ctx?.tq?.result && ctx.tq.result.type === 'table'">
    ...
    <div class="tq-scroll-x" ref="tqScrollXRefLocal">
...
.tq-table-fixed { display: flex; flex-direction: column; width: 100%; flex: 1 1 0%; min-height: 0; }
.tq-scroll-x { position: sticky; top: 0; z-index: 10; background: #fff; border-bottom: 1px solid #e5e7eb; max-width: 100%; overflow-x: auto; overflow-y: hidden; }
.tq-head-inner { display: inline-block; min-height: 36px; height: 36px; background: #fff; }
.tq-head-inner table { height: 36px; }

.tq-body { width: 100%; max-width: 100%; flex: 1 1 0%; min-height: 0; max-height: 100%; overflow-x: auto; overflow-y: auto; -webkit-overflow-scrolling: touch; }
...
.tq-table .th-title { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; }
